...
describe("AttendanceView", () => {
  beforeEach(() => {
    vi.stubGlobal(
      ...
      vi.fn((url: string, init?: RequestInit) => {
        ...
        return Promise.resolve(new Response(JSON.stringify({ students: roster }), { status: 200 }));
      })
    );
  });
  ...
  it("cycles a card's status through null -> present -> absent on repeated clicks", async () => {
    ...
    await userEvent.click(screen.getByLabelText("Attendance for Asha Verma, currently Unmarked"));
