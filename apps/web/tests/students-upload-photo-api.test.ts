...
describe("/api/students/upload-photo", () => {
  ...
  it("saves a valid PNG upload and returns a photoUrl under /uploads/students/", async () => {
    ...
    const response = await postUploadPhoto(
      new Request("http://localhost/api/students/upload-photo", { method: "POST", body: formData })
    );
  ...
  it("rejects a file over the 2MB size limit with 400", async () => {
    ...
    const formData = new FormData();
    formData.append("file", new File([new Uint8Array(2 * 1024 * 1024 + 1)], "big.png", { type: "image/png" }));
  ...
  it("rejects a teacher with 403", async () => {
    ...
    const formData = new FormData();
    formData.append("file", new File([new Uint8Array([1, 2, 3])], "photo.png", { type: "image/png" }));
