...
describe("timetable lib", () => {
  ...
  it("rejects editTimetableEntry when the new subjectId doesn't belong to the entry's class's grade", async () => {
    ...
    const gradeA = await prisma.grade.create({ data: { schoolId: school.id, name: "Grade A", sortOrder: 1 } });
    const gradeB = await prisma.grade.create({ data: { schoolId: school.id, name: "Grade B", sortOrder: 2 } });
