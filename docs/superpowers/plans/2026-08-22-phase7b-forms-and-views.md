...
## Addendum: follow-ups surfaced by Phase 7a's final review, not yet covered by this plan

Phase 7a's final whole-branch review (2026-08-23) found four gaps that this plan does not currently address. None blocked Phase 7a's merge — each is a natural consequence of "nothing is wired to a form yet," which is explicitly this phase's job — but none of the six tasks above pick them up either, so record them here rather than let them fall through the 7a/7b seam silently:

1. **`Grade.sortOrder` has no edit path anywhere.** `createGrade` accepts an optional `sortOrder` (auto-assigned via max+1 if omitted), but `/api/grades` POST never reads it from the request body, and `editGrade` cannot change it at all once a grade exists. This plan's Task on `GradeDetailView`/`ClassesView` only covers subject/class metadata, not grade creation or reordering. Until this is wired, grades sort correctly only by creation order — an admin who creates "Grade 10" before "Grade 2" reproduces the exact alphabetical-sort bug 7a's Task 3 existed to fix, with no in-product way to correct it.
2. **`confirmPromotionRun` bypasses `Class.capacity` entirely.** The two manual paths (`createStudent`, `editStudent`) enforce capacity; the bulk promotion path does not. Currently latent (capacity can't be set via any form yet), but once this plan's class-capacity form task lands, a promotion run can silently overfill a class that the manual paths would then refuse to touch further — an inconsistent state worth an explicit decision (block the run, or document the exemption) rather than silence.
3. **Capacity checks in `createStudent`/`editStudent` are TOCTOU-racy** — the enrollment count is read, then written, in separate transactions, with no exclusion constraint as a backstop. Two concurrent enrollments into a class with one seat left can both pass. Once capacity becomes reachable from a real form (this plan's own Task on `ClassesView`), consider the Serializable-transaction-plus-retry pattern already established in `grades.ts`/`fee-payments.ts`.
4. **`SyllabusVersion.isCurrent` was write-only as of 7a** — this plan's "current syllabus version carries a badge" exit criterion already covers reading it, so this one is expected to close naturally as part of the work above; noted here only for completeness, not as an open gap.

Items 1-3 are not assigned to any task above. Consider whether they belong in this phase (e.g. a grade-creation/reorder form, alongside the existing class-capacity task) or should be tracked separately.
